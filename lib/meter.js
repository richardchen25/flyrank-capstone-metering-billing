...
async function loadTenant(client, tenantId, lock) {
  const result = await client.query(
    `SELECT t.id,
            ...
            s.stripe_subscription_id IS NOT NULL AS period_from_stripe
       FROM tenants t
...
async function getUsage({ tenantId }) {
  ...
  try {
    const tenant = await loadTenant(client, tenantId, false);
    ...
    const costResult = await client.query(
      `SELECT COALESCE(SUM(cost_micros), 0) AS cost_micros
        ...
        WHERE tenant_id = $1
          AND created_at >= $2
          AND created_at < $3`,
      [tenantId, tenant.period_start, tenant.period_end]
    );

    const remaining = remainingQuota(tenant, used.apiCalls, used.tokens);

    return {
      tenantId: tenant.id,
      plan: { id: tenant.plan_id, name: tenant.plan_name },
      subscriptionStatus: tenant.subscription_status,
      period: {
        start: tenant.period_start,
        end: tenant.period_end,
        source: tenant.period_from_stripe ? 'stripe_subscription' : 'calendar_month_fallback',
      },
      apiCalls: {
        used: used.apiCalls,
        limit: Number(tenant.api_calls_limit),
        remaining: remaining.apiCalls,
      },
      tokens: {
        used: used.tokens,
        limit: Number(tenant.tokens_limit),
        remaining: remaining.tokens,
      },
      costMicros: Number(costResult.rows[0].cost_micros),
    };
  } finally {
    client.release();
  }
...
async function recordUsage({ tenantId, eventType, quantity = 0, tokens = {}, idempotencyKey }) {
  ...
  try {
    ...
    tenant = await loadTenant(client, tenantId, true);
